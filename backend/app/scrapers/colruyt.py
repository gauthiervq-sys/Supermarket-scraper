    ...
    try:
        async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
            ...
            if DEBUG_MODE:
                ...
                logger.debug(f"  Colruyt: HTML content length: {len(response.text)} characters")
            ...
            if DEBUG_MODE:
                ...
                if body:
                    ...
                    logger.debug(f"  Colruyt: Page body text (first 500 chars): {body_text[:500]}")
                    logger.debug(f"  Colruyt: Page body text (last 500 chars): {body_text[-500:]}")
                else:
                    logger.debug(f"  Colruyt: No <body> tag found in HTML")
                ...
                if any(indicator in response.text for indicator in ['window.__', 'React', 'Vue', 'Angular', '__NEXT_DATA__']):
                    logger.debug(f"  Colruyt: ⚠️  Site appears to use JavaScript rendering")
            ...
            if DEBUG_MODE:
                ...
                if len(cards) == 0:
                    ...
                    logger.debug(f"  Colruyt: First 20 HTML elements found: {', '.join(element_tags)}")
                    # Show the raw HTML structure (first 1000 chars)
                    logger.debug(f"  Colruyt: Raw HTML structure (first 1000 chars): {response.text[:1000]}")
